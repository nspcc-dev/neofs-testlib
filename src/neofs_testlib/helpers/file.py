...
class FileHelper:
    def __init__(self, assets_dir: str, simple_obj_size: int):
        self.assets_dir = assets_dir
        self.simple_obj_size = simple_obj_size

    def generate_file(self, size: Optional[int]) -> str:
        ...
        """
        size = size or self.simple_obj_size
        file_path = os.path.join(os.getcwd(), self.assets_dir, str(uuid.uuid4()))
    ...
    def generate_file_with_content(
        ...
        mode = "w+"
        if content is None:
            content = os.urandom(self.simple_obj_size)
            mode = "wb"
    ...
    @staticmethod
    def get_file_hash(file_path: str, length: Optional[int] = None) -> str:
        ...
        with open(file_path, "rb") as out:
            if length:
                file_hash.update(out.read(length))
            else:
                file_hash.update(out.read())
        return file_hash.hexdigest()

    def concat_files(self, file_paths: list, resulting_file_path: Optional[str] = None) -> str:
        ...
        if not resulting_file_path:
            resulting_file_path = os.path.join(os.getcwd(), self.assets_dir, str(uuid.uuid4()))
