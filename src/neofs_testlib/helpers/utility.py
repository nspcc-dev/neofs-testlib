...
class UtilityHelper:
    def __init__(self, storage_gc_time: str):
        self.storage_gc_time = storage_gc_time
    ...
    def wait_for_gc_pass_on_storage_nodes(self) -> None:
        wait_time = self.parse_time(self.storage_gc_time)
        logger.info(f"Wait {wait_time}s until GC completes on storage nodes")
